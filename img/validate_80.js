...
function se_ent(_val_){
	...
	if(validate("valid-box") && _val_ == "coupid"){				
		...
		$.ajax({
			...
			type: "GET",
			data: {
				buyby : "^80^"+_product_id+"^"+_couponid_ + "^" + CryptoJS.enc.Base64.parse(CryptoJS.AES.encrypt("^"+_coup_user_ + "^" + _coup_pass_ + "^" +_buyer_user_id_+ "^" + _buyer_user_+"^", kparam , { iv : kparam } ).toString() ).toString(CryptoJS.enc.Hex)
			}
		}).done(function( html ) {
			$( "#message" ).show();
