...
function se_ent(_val_){
	...
	if(validate("valid-box") && _val_ == "coupid"){				
		...
		$.ajax({
			...
			type: "GET",
			data: {
				buyby : "^10^"+_product_id+"^"+_couponid_ + "^" + CryptoJS.enc.Base64.parse(CryptoJS.AES.encrypt("^"+ _coup_user_ + "^" + _coup_pass_ + "^" + _valid_email_+"^", kparam , { iv : kparam } ).toString() ).toString(CryptoJS.enc.Hex)
			}
		}).done(function( html ) {
			$("div#message").css("display","block");
